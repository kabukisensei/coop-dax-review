...
surface keeps resolving (so `from coop_dax_review.<shim> import X` stays valid)."""
...
def test_build_plan_binds_this_package_and_version():
    ...
    from coop_dax_review.upgrade import PACKAGE_NAME, build_plan

    assert PACKAGE_NAME == "coop-dax-review"
    plan = build_plan(
    ...
    assert plan.package_name == "coop-dax-review"
    assert plan.tool_installed == __version__


def test_upgrade_command_carries_this_tool_name():
    from coop_dax_review.upgrade import UpgradePlan, upgrade_command

    plan = UpgradePlan(
        package_name="coop-dax-review", install_method="pipx", checkout=None, tool_installed="x", tool_note=""
    )
...
def test_shim_reexports_still_resolve():
    ...
    from coop_dax_review.diagnostics import (  # noqa: F401
        BASELINE_STALE,
        CONFIG_UNKNOWN_RULE,
    ...
    from coop_dax_review.finding import SEVERITIES, at_or_above, severity_rank  # noqa: F401
    from coop_dax_review.progress import Progress, Tick, should_enable  # noqa: F401
    from coop_dax_review.standards import (  # noqa: F401
        RuleConfig,
        StandardsError,
        add_ignores,
        apply_config,
    ...
    from coop_dax_review.suppressions import (  # noqa: F401
        is_inline_suppressed,
        load_baseline,
        scan_directives,
        write_baseline,
    )
    from coop_dax_review.upgrade import (  # noqa: F401
        DependencyStatus,
        UpgradeError,
        UpgradePlan,
        build_plan,
