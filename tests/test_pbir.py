...
def _create_mock_report(path: Path):
    ...
    visuals1.joinpath("visual.json").write_text(json.dumps({
        "visual": {
            "visualType": "barChart",
            "projections": {
        ...
        "filterConfig": {
            "filters": [
                {"Entity": "DimDate", "Property": "MissingDate"}
            ]
        }
    }))
    ...
    visuals2.joinpath("visual.json").write_text(json.dumps({
        "visual": {
            "visualType": "table",
            "projections": {
                "values": [{"queryRef": "Sales.UsedMeasure", "Property": "UsedMeasure", "Entity": "Sales"}]
            }
        }
    }))
...
def test_parse_pbir_and_rules(tmp_path):
    ...
    ctx_broken = RuleContext(Rule(id="R1", title="", severity="warning", category="", standard_ref="", tier=1), catalog)
    findings = check_broken(ctx_broken)
    
    assert len(findings) == 1
    assert "dimdate[missingdate]" in findings[0].message
    
    ctx_unused = RuleContext(Rule(id="R2", title="", severity="info", category="", standard_ref="", tier=1, kind="agent"), catalog)
