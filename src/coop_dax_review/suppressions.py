...
directive marker (``coop-dax-review:ignore``) so the CLI call sites stay terse.
"""
...
# deliberately can't represent. (Mirrors the coop-sql-review twin.)
_IGNORE_DIRECTIVE_RE = re.compile(rf"(?<![\w-]){re.escape(TOOL)}\s*:\s*ignore\b([^\n]*)", re.IGNORECASE)


def scan_directives(text: str) -> dict[int, set[str]]:
    """Lines carrying a ``coop-dax-review:ignore`` directive -> the rule ids silenced."""
    return _core.scan_directives(text, TOOL)


def scan_syntax_ignores(text: str) -> set[int]:
    """1-based lines whose ``coop-dax-review:ignore`` directive silences a syntax error.

    Fires for an explicit ``syntax`` token (``coop-dax-review:ignore syntax``) or
    ...
    comment text is ignored, matching the rule-id scanner. (Mirrors the
    coop-sql-review twin; DAX comment markers ``//`` / ``--`` are both handled.)
    """
    out: set[int] = set()
