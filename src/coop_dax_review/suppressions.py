"""Suppressions: inline ``coop-dax-review:ignore`` directives + a fingerprint baseline.

Both let this advisory linter be adopted on an existing model without drowning in
already-known findings — and both stay deterministic and never block:

- **inline**: a comment ``coop-dax-review:ignore DAX-VAR-RETURN`` on a finding's
  line (or the line directly above it) silences that rule there. List several ids
  (``ignore DAX-A, DAX-B``) or none / ``*`` to silence all rules on that line; a
...
def is_inline_suppressed(rule_id: str, line: int, directives: dict[int, set[str]]) -> bool:
    ...
    if not line:  # model-/file-level findings (line 0) can't be inline-targeted
        return False
