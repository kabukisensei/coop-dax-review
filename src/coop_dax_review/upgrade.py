"""Tool self-update and dependency freshness (`coop-dax-review upgrade`).
...
def pip_install_origin() -> str | None:
    ...
    ``pip install -U coop-dax-review`` would hit PyPI (where the package may
...
def detect_install_method() -> tuple[str, Path | None]:
    ...
    for candidate in [prefix, *prefix.parents]:
        ...
        try:
            if pyproject.is_file() and 'name = "coop-dax-review"' in pyproject.read_text(
